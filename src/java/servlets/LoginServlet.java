    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (path) {
            case "/login":
                ...
                if(authUser == null){
                    job.add("info", "WE DO NOT HAVE THAT USER!")
                       .add("auth", false);
                ...
                job.add("info", "HELLO THERE, " + authUser.getFirstName() + "!")
                   .add("auth",true)
            ...
            case "/buyModel":
                ...
                if(user.getMoney() >= model.getPrice() && model.getAmount() != 0) {
                    ...
                    job.add("info", "THE PURCHASE WAS SUCCESSFUL!");
                    try (PrintWriter out = response.getWriter()) {
                        out.println(job.build().toString());
                    }
                    break;
                ...
                if(model.getAmount() < 1) {
                    job.add("empty", true);
                    job.add("info", "THERE ARE NO INSTANCES OF THIS MODEL!");
                    try (PrintWriter out = response.getWriter()) {
                        out.println(job.build().toString());
                    }
                    break;
