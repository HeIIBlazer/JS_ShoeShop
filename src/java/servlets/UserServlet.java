        ...
        switch (path) {
            case "/registration":
                ...
                if(firstName.isEmpty() || lastName.isEmpty() 
                    ...
                    job.add("info", "FILL IN ALL THE FIELDS!")
                ...
                if(decimalMoney == 0.0) {
                    job.add("info", "ENTER AMOUNT GREATER THAN ZERO!")
                    .add("firstName", firstName)
                       .add("lastName", lastName)
                       .add("username", username)
                       .add("password", password)
                ...
                job.add("info", "ACCOUNT " + username + " SUCCESFULLY CREATED!")
                        .add("status", true);
                try(PrintWriter out = response.getWriter()) {
                    out.println(job.build().toString());
                }
...
//                }
            ...
            case "/getUser":
                ...
                job.add("status", true)
                    .add("info", "YOU EDITING: " + editingUser.getFirstName() + " " + editingUser.getLastName())
...
//                editUser.setPassword(userPassword);
                ...
                ujb = new UserJsonBuilder();
                job.add("status", true)
                    .add("info", "USER " + editUser.getFirstName() + " " + editUser.getLastName() + " EDITED")
            ...
            case "/changeRole":
                ...
                if("SECONDADMIN".equals(changingRoleUser.getRole())) {
                    job.add("info", "NOW " + changingRoleUser.getFirstName() + " " + 
            ...
            case "/editMyself":
                ...
                try {
                    ...
                    ujb = new UserJsonBuilder();
                    job.add("status", true);
                    job.add("info", "YOURE DATA WAS EDITED SUCCESFULLY!");
                    ...
                    job.add("status", false);
                    job.add("info", "COULD NOT CHANGE THE DATA!");
                    try(PrintWriter out = response.getWriter()) {
                        out.println(job.build().toString());
                    }
                    break;
