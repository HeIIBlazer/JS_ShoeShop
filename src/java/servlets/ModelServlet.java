    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch(path) {
            case "/createModel":
                ...
                if(amount < 1) {
                    job.add("info", "ENTER A QUANTITY GREATER THAN ZERO!");
                    try(PrintWriter out = response.getWriter()) {
                        out.println(job.build().toString());
                    }
                    break;
                ...
                job.add("info", "MODEL " + modelName + " SUCCSESFULLY ADDED!")
                        .add("status", true);
                try(PrintWriter out = response.getWriter()) {
                    out.println(job.build().toString());
                }
            ...
            case "/getModel":
                ...
                job.add("status", true)
                    .add("info", "YOU EDITING: " + editingModel.getModelFirm() + " " + editingModel.getModelName())
            ...
            case "/editModel":
                ...
                mjb = new ModelJsonBuilder();
                job.add("status", true)
                    .add("info", "MODEL " + editModel.getModelFirm() + " " + editModel.getModelName() + " CHANGED")
